...
export const textPages: TextbookPage[] = [
	{
		...
		content: `<p><strong>Transformers</strong> are the engines behind today’s AI, powering tools like ChatGPT and Gemini. They learn from a lot of text and then use that knowledge to write answers, stories, or code. In this guide we’ll look at GPT-2 (small). It’s not the newest model, but it’s simple and perfect for learning the basics.</p>`,
		on: () => {},
		out: () => {}
	},
	{
	...
	},
	{
		...
		content: `
	<p>To perform self-attention, each token's embedding is turned into 
...
This transformation is done by applying numbers (weights and biases) the model learns during training.</p>
...
`,
	...
	},
	{
		...
		content:
			'<p>The attention output goes through an <strong>MLP</strong> to further refine token representations.</p><p>First, the embeddings are changed and resized. Then a simple rule, called activation, decides which numbers to keep or shrink. GPT-2 uses <strong>GELU</strong>, which gently passes small numbers and fully passes big ones. This helps the model capture both subtle and strong patterns.</p>',
		on: () => {
	...
	},
	{
		...
		content:
			'<p>Finally, we need a strategy to pick the next token. Many exist, but here are common ones: Greedy search picks the top one. <strong>Top-k</strong> keeps only the k most likely tokens, and <strong>top-p</strong> keeps the smallest set whose total probability is at least p.</p><p>Then softmax turns the remaining logits into probabilities, and one token is picked at random from the allowed set.</p>',
		on: function () {
			if (get(expandedBlock).id !== 'softmax') {
				expandedBlock.set({ id: 'softmax' });
				this.timeoutId = setTimeout(() => {
	...
	},
	{
		id: 'dropout',
		title: 'Dropout',
		content: `<p>During training, <strong>dropout</strong> randomly removes some connections so the model doesn’t memorize too narrowly. This helps it learn patterns that generalize better. GPT-2 uses it, but many newer models skip it since they train on much larger data and don’t overfit as easily. Dropout is always turned off during inference.</p>`,
		on: () => {
