...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q_weights = q_weights.view(self.n_head, C // self.n_head, C) 
        k_weights = k_weights.view(self.n_head, C // self.n_head, C) 
        v_weights = v_weights.view(self.n_head, C // self.n_head, C) 

        
        q_bias, k_bias, v_bias = self.c_attn.bias.split(self.n_embd, dim=0)
        
        q_bias = q_bias.view(self.n_head, -1) 
        k_bias = k_bias.view(self.n_head, -1) 
        v_bias = v_bias.view(self.n_head, -1) 
        ...
        for i in range(self.n_head):
            ...
            self.dict[f"head_{i}"]["attn"] = attn[:, i]                  
            self.dict[f"head_{i}"]["attn_scaled"] = attn_scaled[:, i]    
            self.dict[f"head_{i}"]["attn_masked"] = attn_masked[:, i]    
